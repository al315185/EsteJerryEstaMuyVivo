		...
		public void UpdateMap(){
			...
			for (int i = 0; i < m_OriginatorList.Count; i++) {
				...
				for (int j = 0; j < influencePositions.Length; j++) {
					...
					// si esa posicion está dentro del grid (index out of range)
...
//					Debug.Log(InfluenceMapTexture.GetPixel(i,j).r);
//					Debug.Log(InfluenceMapTexture.GetPixel(i,j).g);
//					Debug.Log(InfluenceMapTexture.GetPixel(i,j).b);
